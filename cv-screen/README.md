...
Cost is charged on input tokens only, output is free, at the published price of $42 per billion input tokens. One accounting note the two examples differ on: `scripts/bench.ts` prices every token it counts at that input rate, so example 1 is a mild
...
Run B, after changing one setting: the sector question switched on and labelled `frontend development with React or React Native` (weight 0.3), experience raised to 0.3, stability dropped
